...
INSERT INTO config (id, value) VALUES (8, 10000); -- cloud
...
BEGIN
  ...
  DELETE FROM cloud WHERE seq > ( SELECT value FROM config WHERE id = 8 );
END;
...
END;
CREATE TRIGGER "tr_cloud" AFTER INSERT ON "cloud"
BEGIN
  update cloud set seq = id % (SELECT value FROM config WHERE id = 8) where id=new.id;
END;
