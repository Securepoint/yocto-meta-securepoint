...
DEPENDS = "net-snmp openssl libusb"
...
EXTRA_OECONF += '--with-lib --with-usb --with-snmp --with-snmp-libs="-L${STAGING_LIBDIR} -lnetsnmp" --with-snmp-includes="-I${STAGING_INCDIR}" --without-doc \
--with-user=ups --with-group=nut \
--with-pidpath=/var/run/nut --with-altpidpath=/var/run/nut --with-statepath=/var/run/nut'
